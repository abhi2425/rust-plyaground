...
fn partial_move() {
    ...
    println!("The person-2 struct is {:?}", person2);
...
fn mutability() {
    #[derive(Copy, Clone)] // try commenting this line
    struct Book {
    ...
    // Create a mutable copy of `immutable_book` and call it `mut_book`
...
fn aliasing() {
    ...
    // it is possible to re_borrow with a mutable reference.
    let mutable_borrow = &mut point;
    // borrowed_point;
